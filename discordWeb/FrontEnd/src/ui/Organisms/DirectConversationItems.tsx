...
import { getConversationList, UnauthorizedError } from "../../helpers/helpers";
...
function DirectConversationItems() {
  ...
  const { jwtToken, setJwtToken, conversationList, setConversationList } = ctx;
  ...
  const groupedConversations = useMemo(() => {
    ...
    const grouped = conversationList.reduce((acc, item) => {
      const existing = acc.find(c => c.conversationId === item.conversationId);
