...
function NavbarButton({ buttonText }: { buttonText: string }) {
  ...
  const { selectedNavbarButton, setSelectedNavbarButton } = ctx;
  
  // console.log("navbar : ", selectedNavbarButton, "button text : ", buttonText);
  return (
    <button 
      onClick={() => setSelectedNavbarButton(buttonText.toLowerCase().trim())}
      className={` p-2  rounded-xl text-gray-300 ${
        ...
        buttonText.toLowerCase().trim() != "arkadaş ekle" && " text-white bg-[#404044]"
         
      }
      ${
        selectedNavbarButton.toLowerCase().trim() === "arkadaş ekle"
          ? buttonText.toLowerCase().trim() == "arkadaş ekle" && "bg-[#232540] !text-[#7A80CA] hover:bg-[#232540] !cursor-default "
          : " "
      }  `}
    >
      {buttonText}
